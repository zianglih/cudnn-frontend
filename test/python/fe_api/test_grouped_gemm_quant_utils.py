...
def run_grouped_gemm_quant_ref(
    ...
    # Step 3: optional row-scaled NVFP4 row scale, also called 1D2D in
    # block-scaled FP8 convention. This intentionally scales only the GEMM
    # accumulator path; fused bias keeps its existing bias * prob semantics.
    # TransformerEngine's cpp_extensions/gemm.py folds the row-scaled input and
    # tensor-scaled input global decode scales into this row multiplier.
