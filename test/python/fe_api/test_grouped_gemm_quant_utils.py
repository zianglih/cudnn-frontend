...
def run_grouped_gemm_quant_ref(
    ...
    # Step 3: optional row-scaled NVFP4 per-token scale.  This intentionally
    # scales only the GEMM accumulator path; fused bias keeps its existing
    # bias * prob semantics.
    if per_token_scale_tensor is not None:
        ref = ref * per_token_scale_tensor[:valid_m].to(torch.float32).view(valid_m, 1, 1)
